...
> Git hash: 4de2e67c6d193645e198dd5947f4bb921749495e
...
| `test-agent.md` | 4 | implementation-agent (validator mode Gate 4), df-intake (advisor mode Step 5.5), contracts test, setup test |
...
| `dark-factory/promoted-tests.json` | 4 | promote-agent (writes), df-cleanup (reads for health check + --rebuild-memory), setup test (validates structure), test-agent (reads — Step 2.75 regression gate) |
...
**New high fan-in (4de2e67)**: `test-agent.md` now spawned by both implementation-agent (validator mode) and df-intake (advisor mode Step 5.5); `dark-factory/promoted-tests.json` now read by 4 callers.
...
- `test-agent.md` [judge] -> [project-profile.md, code-map.md, holdout scenarios (validator), promoted-tests.json (Step 2.75), memory/index.md + INV shards (advisor), playwright.config.*, dark-factory/results/]

### .claude/skills/
- `df/SKILL.md` -> [df-intake/SKILL.md, df-debug/SKILL.md]
- `df-intake/SKILL.md` -> [code-map.md, codemap-agent.md, spec-agent.md, qa-agent.md, test-agent.md (advisor mode Step 5.5), manifest.json]
...
- `df-cleanup/SKILL.md` -> [project-profile-slim.md, code-map-slim.md, templates/project-profile-slim-template.md, manifest.json, promoted-tests.json, promote-agent.md, dark-factory/memory/ (--rebuild-index/--rebuild-memory)]
- `df-spec/SKILL.md` -> []
...
- `manifest.json` — read/written by skills + implementation-agent; tracks `testAdvisoryCompleted` (new)
- `project-profile.md`, `project-profile-slim.md` — written by onboard-agent; read by all agents
- `promoted-tests.json` — written by promote-agent; 16 entries
...
- `memory/index.md`, `memory/invariants-*.md`, `memory/decisions-*.md`, `memory/design-intent-*.md`, `memory/ledger.md` — 11 memory files; written by onboard-agent + promote-agent; df-cleanup --rebuild-index maintenance exception for index.md
